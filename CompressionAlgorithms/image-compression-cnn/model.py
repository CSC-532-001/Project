...
def print_model_params(verbose=True):
    ...
    for variable in tf.trainable_variables():
        ...
        if verbose:
            print("variable parameters: ", variable_parametes)
...
class CNN:
    ...
    def conv2d_depth_or_not(self, input_, name, nonlinearity=None):
        with tf.variable_scope(name) as scope:
            ...
            conv_weights = tf.get_variable(
                "W", shape=W_shape, initializer=W_initializer
            )
            ...
            if name == "depth":
                ...
                conv = tf.nn.depthwise_conv2d_native(
                    input_, conv_weights, [1, 1, 1, 1], padding="SAME"
                )
    ...
    def fully_connected_layer(
        self, input_, input_size, output_size, name, nonlinearity=None
    ):
        ...
        with tf.variable_scope(name) as scope:
            ...
            b = tf.get_variable(
                "b", shape=[output_size], initializer=tf.constant_initializer(0.0)
            )
    ...
    def build(self, image):
        ...
        pool1 = tf.nn.max_pool(
            conv1_2,
            ksize=cnn_param.pool_window,
            strides=cnn_param.pool_stride,
            padding="SAME",
            name="pool1",
        )
    ...
    def get_classmap(self, class_, conv_last):
        ...
        conv_last_ = tf.reshape(
            conv_last_, [-1, hyper.image_h * hyper.image_w, cnn_param.last_features]
        )
        classmap = tf.reshape(
            tf.matmul(conv_last_, class_w), [-1, hyper.image_h, hyper.image_w]
        )
        return classmap
