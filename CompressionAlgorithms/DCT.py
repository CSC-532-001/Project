...
def rgbToYCBCR(image):
    # These numbers are a part of a formula, provided by ITU-R BT.601 standard, used in SDTV (Standard-Definition Television). Provided by ChatGPT.
    nform = np.array(
        [
    ...
    imageYCBCR[
        :, :, [1, 2]
    ] += 128  # Adds to Cb and Cr in every pixel, and normalizes chrominance channels.
    return np.uint8(
        imageYCBCR
...
def compressImage(image, blockSize=int(8), quality=int(50)):
    ...
    (
        ...
        paddedImage.shape
    )  # height, width, _ = image.shape # Assigns variables to the tuple of .shape from NumPy
    compressedImage = np.zeros(
...
def padImageToBlockSize(image, blockSize):
    ...
    paddedImage = np.pad(image, ((0, padHeight), (0, padWidth), (0, 0)), "edge")
    return paddedImage
