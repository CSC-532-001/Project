...
def compress(img, ratio):
    ...
    flattened_coeffs = np.concatenate([c.flatten() for level_coeffs in coeffs for c in level_coeffs])
        ...
        for c in level_coeffs:
            if isinstance(c, tuple):
                for sub_c in c:
                    ...
                    level_compressed_coeffs.append(flattened_coeffs[idx : idx + flat_len].reshape(sub_c.shape))
                    idx += flat_len
            else:
                flat_len = np.prod(c.shape)
                level_compressed_coeffs.append(flattened_coeffs[idx : idx + flat_len].reshape(c.shape))
                idx += flat_len
